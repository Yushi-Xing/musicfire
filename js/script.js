...
function toggleMusic(toggle) {
	if (typeof toggle === 'boolean') {
		store.setState({ musicEnabled: toggle });
	} else {
		store.setState({ musicEnabled: !store.state.musicEnabled });
	}
	var mu=document.querySelector("#music-dom");
	if(store.state.musicEnabled)
	{
		mu.play();
	}
	else{
...
const helpContent = {
		shellType: {
		header: '烟花类型',
		body: '将要发射的烟花的类型。选择"随机"以获得一个漂亮的组合！Random: 随机 Crackle: 散花 Crossette: 十字花 Crysanthemum: 菊花 Falling Leaves: 落叶 Floral: 花朵 Ghost: 幽灵 Horse Tail: 马尾 Palm: 棕榈 Ring: 环形 Strobe: 闪光 Willow: 柳树'
		},
		shellSize: {
		header: '烟花尺寸',
		body: '烟花的大小。模仿真实的烟花外壳大小，较大的外壳有更大的爆炸，更多的星星，有时还有更复杂的效果。然而，较大的外壳也需要更多的处理能力，可能会导致卡顿。'
		},
		quality: {
		header: '质量',
		body: '整体图形质量。如果动画运行不流畅，请尝试降低质量。高质量会大大增加火花的渲染数量，可能会导致卡顿。'
		},
		skyLighting: {
		header: '天空照明',
		body: '烟花爆炸时照亮背景。如果屏幕上的背景看起来太亮，请尝试将其设置为"昏暗"或"无"。'
		},
		scaleFactor: {
		header: '缩放',
		body: '允许调整所有烟花的大小，实际上是让您靠近或远离烟花。对于较大的外壳大小，可以适当减小缩放比例，特别是在手机或平板电脑上。'
		},
		autoLaunch: {
		header: '自动发射',
		body: '自动发射烟花序列。坐下来享受烟花表演，或禁用以完全控制烟花发射。'
		},
		finaleMode: {
		header: '快速模式',
		body: '发射强烈的烟花爆炸。可能会导致卡顿。需要启用"自动发射"。'
		},
		hideControls: {
		header: '隐藏控件',
		body: '隐藏屏幕顶部的半透明控件。对于截图很有用，或者只是更流畅的体验。在隐藏状态下，您仍然可以点击屏幕右上角重新打开此菜单。'
		},
		fullscreen: {
		header: '全屏',
		body: '切换全屏模式。'
		...
		longExposure: {
		header: '开启长曝光',
		body: '实验性效果，保留长时间的光迹，类似于将相机快门打开。'
		},
		firesound: {
			header: '开启烟花音效',
			body: '烟花发射和爆炸音效'
...
const appNodes = {
	...
	firesound:".firesound",
	firesoundlabel:".firesound-label",
	BGM:".BGM",
	BGMlabel:".BGM-label",
	// Help UI
...
function renderApp(state) {
	...
	const musicBtnIcon = `#icon-music-${state.musicEnabled ? 'on' : 'off'}`;

	appNodes.musicBtnSVG.setAttribute('href', musicBtnIcon);
	appNodes.musicBtnSVG.setAttribute('xlink:href', musicBtnIcon);
	...
	appNodes.firesound.checked=state.soundEnabled;
	appNodes.BGM.checked=state.musicEnabled;
...
function getConfigFromDOM() {
	return {
		...
		firesound: appNodes.firesound.checked,
		BGM:appNodes.BGM.checked,
...
appNodes.firesound.addEventListener('click', () => setTimeout(function(){ toggleSound();}, 0));
appNodes.BGM.addEventListener('click', () => setTimeout(function(){toggleMusic(); }, 0));
...
function handlePointerStart(event) {
	activePointerCount++;
	const btnSize = 50;
	var gsize=(mainStage.width-btnSize*4)/3 ;
	if (event.y < btnSize) {
		if (event.x < btnSize) {
			togglePause();
			return;
		}
		if (event.x > gsize+btnSize && event.x < gsize+btnSize*2) {
			toggleSound();

			return;
		}

		if (event.x > 2*gsize+2*btnSize && event.x < 2*gsize+btnSize*3) {
